...
tasks.register("verifyDependencyResolution") {
  ...
  doLast {
    ...
    val bootComponents =
        resolvedLog4j.filter { it.startsWith(":j-store-boot\t") }.map { it.split('\t')[1] }.toSet()
    ...
    archiveFiles.forEach { archive ->
      ZipFile(archive).use { zip ->
        ...
        if (entry == null) {
          ...
          violations += "${archive.relativeTo(rootDir)}: META-INF/LICENSE differs from root LICENSE"
        }
        ...
        if (thirdPartyEntry == null) {
          violations += "${archive.relativeTo(rootDir)}: missing META-INF/THIRD_PARTY.md"
            ...
            !zip.getInputStream(thirdPartyEntry).readBytes().contentEquals(expectedThirdPartyNotice)
        ) {
          violations +=
