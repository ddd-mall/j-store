...
- 新增 Category、Brand、LocalizedText 和 MediaAsset 领域基础类型；SPU 和快照保存品牌/类目引用及本地化内容。
...
完整质量门禁结果：治理契约、86 个 Python 治理/规格/工具测试、文件所有权、Spotless、所有 Gradle 测试、依赖许可证审计和 53 个发布 JAR 许可证验证全部通过。
...
- Category 和 Brand 已有稳定类型并可被 SPU/快照引用，完整管理用例、层级移动规则和搜索投影属于后续迭代。
