...
- 渲染和部署入口拒绝 tag、绑定显式 context、严格 cluster UID、`jstore` namespace 和
  外部 `jstore-runtime` Secret，并在 apply 前执行 server-side dry-run。
- canary/production 使用双副本、RollingUpdate、PDB、拓扑分散、优雅终止和受控入口
  NetworkPolicy；默认 `local` profile 与 PVC/JAR 交付路径已经移除。
...
目标 server-side dry-run、rollout、真实灰度流量或上一 digest 回滚。首个集成流水线必须用
同一候选 digest 完成这些演练并保存 commit、源/目标 digest、cluster UID、配置版本和结果；
