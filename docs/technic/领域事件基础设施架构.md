...
该保证只覆盖同一数据库事务内的副作用。listener 不得直接执行无法随事务回滚的 HTTP、邮件、支付或第三方调用；确需调用时，下游必须以 `eventId` 实现可验证幂等，或者 listener 只在当前事务写入专用 outbox，再由独立 worker 执行外部调用。

relay 的交付语义是 at-least-once。同一 `aggregateType + aggregateId` 以 `(createdAt, id)` 确定顺序：前序记录未进入 `PUBLISHED` 前，后序记录不会被领取；不同聚合仍可并行。
...
| `OutboxDeadLetterService` | common-spring | 死信分页查询、受审计 requeue | 前端管理 UI |
