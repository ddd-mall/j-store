...
class AccountingApplicationService(
    ...
    fun recordOrderPaid(cmd: RecordOrderPaidCMD): Result<JournalEntry, BusinessError> {
        ...
        val period = accountingPeriodRepository.requireOpenPeriod(cmd.accountingDate)
            .let { result -> when (result) {
                is Success -> result.value
                is Failure -> return Failure(result.error)
            } }
        val clearing = requireAccount("1010", SubjectType.CHANNEL, "DEFAULT").let { result ->
            when (result) {
                is Success -> result.value
                is Failure -> return Failure(result.error)
            }
        ...
        entry.addLine(JournalLine(journalEntryRepository.nextLineId(), clearing.id, EntrySide.DEBIT, cmd.paidAmount, "订单支付代收"))
            .onFailure { return Failure(it) }
        entry.addLine(JournalLine(journalEntryRepository.nextLineId(), payable.id, EntrySide.CREDIT, cmd.paidAmount, "商户待结算款"))
            .onFailure { return Failure(it) }
    ...
    fun recordOrderCompleted(cmd: RecordOrderCompletedCMD): Result<JournalEntry, BusinessError> {
        ...
        val commission = requireAccount("3001", SubjectType.PLATFORM, "PLATFORM").let { result ->
            when (result) {
                is Success -> result.value
                is Failure -> return Failure(result.error)
            }
        ...
        val entry = JournalEntryImpl(
            id = journalEntryRepository.nextId(),
            entryNo = journalEntryRepository.nextEntryNo(JournalEntryType.ORDER_COMPLETION_COMMISSION),
            type = JournalEntryType.ORDER_COMPLETION_COMMISSION,
            sourceDocument = cmd.sourceDocument,
            accountingDate = cmd.accountingDate,
        )
        entry.addLine(JournalLine(journalEntryRepository.nextLineId(), payable.id, EntrySide.DEBIT, cmd.commissionAmount, "订单完成确认佣金"))
            .onFailure { return Failure(it) }
        entry.addLine(JournalLine(journalEntryRepository.nextLineId(), commission.id, EntrySide.CREDIT, cmd.commissionAmount, "平台佣金收入"))
            .onFailure { return Failure(it) }
        entry.post(period).onFailure { return Failure(it) }
        return Success(journalEntryRepository.save(entry))
    }
    ...
    fun recordOrderRefundApproved(cmd: RecordOrderRefundApprovedCMD): Result<JournalEntry, BusinessError> {
        ...
        val entry = JournalEntryImpl(
            id = journalEntryRepository.nextId(),
            entryNo = journalEntryRepository.nextEntryNo(JournalEntryType.ORDER_REFUND_REVERSAL),
    ...
    private fun requireAccount(code: String, subjectType: SubjectType, subjectId: String) =
        ledgerAccountRepository.findByCodeAndSubject(
            ...
            ?: Failure(com.jstore.accounting.domain.account.AccountingAccountErrors.ACCOUNT_NOT_FOUND)
}
