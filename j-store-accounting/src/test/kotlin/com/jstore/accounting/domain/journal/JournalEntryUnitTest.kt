...
class JournalEntryUnitTest : FunSpec({
    val openPeriod = AccountingPeriodImpl(
    ...
    fun draftEntry() = JournalEntryImpl(
        id = JournalEntryId(1),
        entryNo = "JE1",
    ...
    test("balanced journal entry can be posted") {
        ...
        entry.addLine(JournalLine(JournalLineId(1), LedgerAccountId(1010), EntrySide.DEBIT, Price.ofFen(1000), "debit"))
        entry.addLine(JournalLine(JournalLineId(2), LedgerAccountId(2101), EntrySide.CREDIT, Price.ofFen(1000), "credit"))
    ...
    test("unbalanced journal entry cannot be posted") {
        val entry = draftEntry()
        entry.addLine(JournalLine(JournalLineId(1), LedgerAccountId(1010), EntrySide.DEBIT, Price.ofFen(1000), "debit"))
        entry.addLine(JournalLine(JournalLineId(2), LedgerAccountId(2101), EntrySide.CREDIT, Price.ofFen(900), "credit"))
    ...
    test("posted journal entry cannot be changed and reversal keeps original lines unchanged") {
        ...
        entry.addLine(JournalLine(JournalLineId(3), LedgerAccountId(3001), EntrySide.CREDIT, Price.ofFen(100), "late"))
            .shouldBe(Failure(AccountingErrors.JOURNAL_ENTRY_ALREADY_POSTED))

        val reversal = (entry.createReversal(JournalEntryId(2), "JE2", LocalDate.of(2026, 4, 30), "reverse") as Success).value
