...
class SettlementStatementUnitTest : FunSpec({
    fun statement() = SettlementStatementImpl(
    ...
    test("settlement payable amount equals sum of net amounts") {
        ...
        statement.addLine(SettlementLine(SettlementLineId(1), "o1", Price.ofFen(1000), Price.ZERO, Price.ofFen(100), Price.ofFen(900)))
        statement.addLine(SettlementLine(SettlementLineId(2), "o2", Price.ofFen(2000), Price.ZERO, Price.ofFen(200), Price.ofFen(1800)))
    ...
    test("paid transition only allowed after confirmation") {
        ...
        statement.markPaid(Instant.now()).shouldBe(Failure(SettlementErrors.SETTLEMENT_STATEMENT_INVALID_STATE))
        statement.confirm()
