...
class FakeJournalEntryRepository : JournalEntryRepository {
    ...
    var savedCount = 0
    val savedEntries: List<JournalEntry> get() = entries.values.toList()
    ...
    override fun nextEntryNo(type: JournalEntryType): String = "${type.name}-${sequence.incrementAndGet()}"
    override fun summarizeBalance(query: AccountingBalanceQuery): List<AccountingBalanceView> = emptyList()
}
...
class FakeLedgerAccountRepository : LedgerAccountRepository {
    ...
    override fun findById(id: LedgerAccountId): LedgerAccount? = accounts.firstOrNull { it.id == id }
    override fun findByCodeAndSubject(code: LedgerAccountCode, subject: AccountingSubject): LedgerAccount? =
        accounts.firstOrNull { it.code == code && it.subject == subject }
