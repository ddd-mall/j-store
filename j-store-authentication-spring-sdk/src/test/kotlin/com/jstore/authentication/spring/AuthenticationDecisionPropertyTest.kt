...
class AuthenticationDecisionPropertyTest :
    FunSpec({

        // **Validates: Requirements 1.2, 1.3, 1.4, 1.5, 2.2, 2.3, 2.4, 2.5, 3.2, 3.3**
        test(
            "requiresAuthentication follows priority rules for any annotation + path combination"
        ) {
            val pathSegments =
                listOf("api", "public", "admin", "users", "orders", "health", "login", "v1", "v2")

            // Generator for realistic URL paths like "/api/users", "/admin/orders"
            val pathArb =
                Arb.list(Arb.element(pathSegments), range = 1..3).map { segments ->
                    "/" + segments.joinToString("/")
                }

            // Generator for path patterns (may include wildcards)
            val patternSegments =
                listOf("api", "public", "admin", "users", "orders", "health", "login", "**", "*")
            val patternArb =
                Arb.list(Arb.element(patternSegments), range = 1..3).map { segments ->
                    "/" + segments.joinToString("/")
                }
            ...
            checkAll(
                ...
                hasSkipLogin,
                hasMethodRequireLogin,
                hasClassRequireLogin,
                requestPath,
                ...
                `when`(handlerMethod.hasMethodAnnotation(SkipLogin::class.java))
                    .thenReturn(hasSkipLogin)
                `when`(handlerMethod.hasMethodAnnotation(RequireLogin::class.java))
                    .thenReturn(hasMethodRequireLogin)

                val beanType: Class<*> =
                    if (hasClassRequireLogin) AnnotatedController::class.java
                    else UnannotatedController::class.java
                ...
                val configurer =
                    object : AuthenticationConfigurer {
                        override fun authenticatedPathPatterns(): List<String> =
                            authenticatedPatterns
                ...
                val matchesExcluded = excludedPatterns.any { pattern ->
                    pathMatcher.match(pattern, requestPath)
                }
                val matchesAuthenticated = authenticatedPatterns.any { pattern ->
                    pathMatcher.match(pattern, requestPath)
                }
...
@RequireLogin class AnnotatedController
