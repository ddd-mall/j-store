...
class AuthenticationInterceptorTest : FunSpec({

    // _需求: 6.4_ — 非预期异常返回 HTTP 500 + Auth.InternalError，不泄露异常详情
    test("unexpected exception during token validation returns HTTP 500 with Auth.InternalError") {
    ...
    test("when both annotation and path config require auth, token validation executes only once") {
        val tokenProvider = mock<TokenProvider>()
        val tokenStore = mock<TokenStore>()
