 ...
 * 超时任务中心的业务门面。
 *
 * <p>业务方通过此接口提交延迟任务，无需关心 slot 计算、Redis 写入等内部细节。
 *
 * <p>使用示例：
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>注意：对应 topic 的 {@link TimerJobHandler} 实现必须保证幂等性（至少一次语义）。
 */
@Slf4j
@Service
public class TimerJobService {
    ...
    public TimerJob submitAt(String topic, String content, Date executeTime) {
        ...
        TimerJob timerJob =
                new TimerJob().setTopic(topic).setContent(content).setExecuteTime(executeTime);

        long slot = jobLoader.slot(timerJob);
        TimerJob created = timerJobRepository.addNewJobAndEnqueue(timerJob, slot);

        log.debug(
                "任务已提交: id={}, topic={}, executeTime={}, slot={}",
                created.getId(),
                topic,
                executeTime,
                slot);
