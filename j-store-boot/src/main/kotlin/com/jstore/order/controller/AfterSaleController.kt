...
class AfterSaleController(private val service:AfterSaleApplicationService){
 ...
 @PostMapping("/{id}/receive-return") fun receiveReturn(@CurrentUserId uid:Long,@PathVariable id:Long)=service.receiveReturn(AfterSaleReceiveReturnCMD(AfterSaleId(id),MerchantActorId(uid))).response()
 @PostMapping("/{id}/retry-refund") fun retryRefund(@CurrentUserId uid:Long,@PathVariable id:Long)=service.retryRefund(AfterSaleRetryRefundCMD(AfterSaleId(id),MerchantActorId(uid))).response()
