...
@Component
class OrderCreatedToPaymentTranslator(
    private val payments: PaymentApplicationService,
    ...
    override fun onDomainEvent(event: OrderCreatedEvent) {
        payments.createForOrder(
            PaymentOrderRequest(event.orderId.value, event.merchantId.value, event.payableAmount, event.currency)
        ).getOrThrow()
    }
}

@Component
class PaymentCapturedToOrderTranslator(
...
class OrderPaidToFulfillmentTranslator(
    ...
    override fun onDomainEvent(event: OrderPaidEvent) {
        val order = requireNotNull(orders.findById(event.orderId)) { "Order ${event.orderId.value} not found" }
        val recipient = order.recipientInfo
        fulfillments.createForOrder(
            FulfillmentRequest(
...
@Component
class FulfillmentPreparedToOrderTranslator(private val orders: OrderService) : DomainEventListener<FulfillmentPreparedEvent> {
    ...
    override fun onDomainEvent(event: FulfillmentPreparedEvent) {
        orders.recordFulfillmentPrepared(OrderId(event.orderId), event.fulfillmentId.value.toString()).getOrThrow()
    }
}

@Component
class ShipmentDispatchedToOrderTranslator(private val orders: OrderService) : DomainEventListener<ShipmentDispatchedEvent> {
    ...
    override fun onDomainEvent(event: ShipmentDispatchedEvent) {
        orders.recordShipmentDispatched(OrderId(event.orderId), event.fulfillmentId.value.toString()).getOrThrow()
    }
}

@Component
class ShipmentDeliveredToOrderTranslator(private val orders: OrderService) : DomainEventListener<ShipmentDeliveredEvent> {
    override fun listenerId() = "translator.shipment-delivered.to-order.v1"
    override fun onDomainEvent(event: ShipmentDeliveredEvent) {
        orders.recordShipmentDelivered(OrderId(event.orderId), event.fulfillmentId.value.toString()).getOrThrow()
        ...
        payments.requestRefund(
            PaymentRefundRequest(
                ...
                items = event.items.map {
                    PaymentRefundItem(it.orderItemId.value, it.skuId, it.quantity, it.amount)
                },
                amount = event.amount,
            )
        ).getOrThrow()
...
class PaymentRefundSucceededToAfterSaleTranslator(
    ...
    override fun onDomainEvent(event: PaymentRefundSucceededEvent) {
        afterSales.recordRefundSucceeded(
...
@Component
class AfterSaleRefundSucceededToOrderTranslator(
    private val orders: OrderService,
