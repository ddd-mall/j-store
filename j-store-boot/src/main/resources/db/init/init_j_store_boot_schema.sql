...
CREATE INDEX IF NOT EXISTS idx_timer_job_execute_time_status
    ON timer_job (execute_time, status);

CREATE TABLE IF NOT EXISTS handled_timer_job (
    id            BIGSERIAL     PRIMARY KEY,
    timer_job_id  BIGINT        NOT NULL,
    topic         VARCHAR(255)  NOT NULL,
    content       TEXT          NOT NULL,
    status        VARCHAR(255)  NOT NULL,
    remind_ttl    SMALLINT      NOT NULL,
    execute_time  TIMESTAMP     NOT NULL,
    create_time   TIMESTAMP,
    CONSTRAINT uk_handled_timer_job_job_id UNIQUE (timer_job_id)
);

CREATE INDEX IF NOT EXISTS idx_handled_timer_job_execute_time_topic
    ON handled_timer_job (execute_time, topic);

CREATE TABLE IF NOT EXISTS timer_job_dead_queue (
    id            BIGSERIAL     PRIMARY KEY,
    timer_job_id  BIGINT        NOT NULL,
    topic         VARCHAR(255)  NOT NULL,
    content       TEXT          NOT NULL,
    ...
    CONSTRAINT uk_timer_job_dead_queue_job_id UNIQUE (timer_job_id)
);

CREATE INDEX IF NOT EXISTS idx_timer_job_dead_queue_execute_time_topic
    ON timer_job_dead_queue (execute_time, topic);
