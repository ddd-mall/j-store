...
CREATE INDEX idx_trades_buyer_status ON trades (buyer_party_type, buyer_party_id, status);
...
CREATE INDEX idx_trade_order_plans_trade_status ON trade_order_plans (trade_id, status);
...
ALTER TABLE orders
    ...
    ADD CONSTRAINT fk_orders_source_trade FOREIGN KEY (source_trade_id) REFERENCES trades(id),
    ADD CONSTRAINT fk_orders_source_order_plan FOREIGN KEY (source_order_plan_id) REFERENCES trade_order_plans(order_plan_id),
