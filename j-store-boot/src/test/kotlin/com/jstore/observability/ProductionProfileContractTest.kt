...
class ProductionProfileContractTest {
  ...
  fun `production requires external connections and secrets`() {
    ...
    val requiredExternalProperties =
        mapOf(
            ...
            "account.phone-verification.hmac-secret" to "\${JSTORE_PHONE_VERIFICATION_HMAC_SECRET}",
        )
    ...
    assertThat(production.getProperty("spring.datasource.hikari.pool-name")).isEqualTo("j-store")
  ...
  fun `local identity and health groups cannot leak outbox into probes`() {
    ...
    assertThat(
            outboxObservability.getProperty("management.endpoint.health.group.operations.include")
        )
        .isEqualTo("outbox")
  }
