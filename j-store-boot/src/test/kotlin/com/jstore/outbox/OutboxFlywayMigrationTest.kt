        ...
        EmbeddedPostgres.builder().start().use { postgres ->
            ...
            dataSource.connection.use { connection ->
                connection.createStatement().use { statement ->
                    ...
                    statement.executeQuery("SELECT to_regclass('develop.outbox_dead_letter_audit')").use { rows ->
                        assertTrue(rows.next())
    ...
    fun `hardening migration preserves existing outbox states`() {
        EmbeddedPostgres.builder().start().use { postgres ->
            ...
            Flyway.configure().dataSource(dataSource).locations("classpath:db/migration")
                .schemas("develop").defaultSchema("develop")
                .target(MigrationVersion.fromVersion("20260803")).load().migrate()
            dataSource.connection.use { connection ->
                connection.createStatement().use { statement ->
                    listOf("PENDING", "FAILED", "IN_PROGRESS", "DEAD_LETTER", "PUBLISHED").forEachIndexed { index, status ->
            ...
            Flyway.configure().dataSource(dataSource).locations("classpath:db/migration")
                .schemas("develop").defaultSchema("develop").load().migrate()
            dataSource.connection.use { connection ->
                connection.createStatement().use { statement ->
                    statement.executeQuery("SELECT status, lock_token FROM develop.outbox_entry WHERE id LIKE 'legacy-%'").use { rows ->
                        val states = mutableSetOf<String>()
                        while (rows.next()) {
                            states += rows.getString("status")
                            assertEquals(0L, rows.getLong("lock_token"))
                        }
                        assertEquals(setOf("PENDING", "FAILED", "IN_PROGRESS", "DEAD_LETTER", "PUBLISHED"), states)
                    }
                }
            }
        }
