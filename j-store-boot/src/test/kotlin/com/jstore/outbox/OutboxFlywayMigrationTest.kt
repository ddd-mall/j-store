...
class OutboxFlywayMigrationTest {
    ...
    EmbeddedPostgres.builder().start().use { postgres ->
      ...
      dataSource.connection.use { connection ->
        connection.createStatement().use { statement ->
          ...
          statement.executeQuery("SELECT to_regclass('develop.outbox_dead_letter_audit')").use {
              rows ->
            assertTrue(rows.next())
          ...
          statement.executeQuery("SELECT to_regclass('develop.outbox_stream_position')").use { rows
            ->
            assertTrue(rows.next())
          ...
          listOf(
              ...
              .forEach { indexName ->
                statement.executeQuery("SELECT to_regclass('develop.$indexName')").use { rows ->
                  assertTrue(rows.next())
                  assertEquals("develop.$indexName", rows.getString(1))
                }
              }
          val removedTimerTables = listOf("timer_job", "handled_timer_job", "timer_job_dead_queue")
  ...
  fun `hardening migration preserves existing outbox states`() {
    EmbeddedPostgres.builder().start().use { postgres ->
      ...
      dataSource.connection.use { connection ->
        connection.createStatement().use { statement ->
          listOf("PENDING", "FAILED", "IN_PROGRESS", "DEAD_LETTER", "PUBLISHED").forEachIndexed {
              index,
              status ->
    ...
    EmbeddedPostgres.builder().start().use { postgres ->
      ...
      dataSource.connection.use { connection ->
        connection.createStatement().use { statement ->
          statement
              .executeQuery(
                  "SELECT id, ordering_key, sequence_no FROM develop.outbox_entry " + "ORDER BY id"
              )
              .use { rows ->
                ...
                assertEquals(
                    "f1a476be28e1db6b844cd08b189fdb4dab3db81545105da5711f10d336aa9193" to 1L,
                    positions["rabbit-1"],
                )
              }
          statement
