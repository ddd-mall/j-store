...
class CartPO(
    ...
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 16) var status: CartStatus = CartStatus.ACTIVE,
    ...
    @OrderBy("id") var lines: MutableList<CartLinePO> = mutableListOf(),
)

@Entity @Table(name = "cart_lines", uniqueConstraints = [UniqueConstraint(columnNames = ["cart_id", "offer_id"])])
class CartLinePO(
    @Id var id: Long = 0,
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "cart_id", nullable = false) var cart: CartPO? = null,
...
@Entity @Table(name = "cart_assessments", uniqueConstraints = [UniqueConstraint(columnNames = ["cart_id", "source_cart_version"])])
class CartAssessmentPO(
    ...
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 16) var status: AssessmentStatus = AssessmentStatus.EMPTY,
    ...
    @OrderBy("cartLineId") var lines: MutableList<CartAssessmentLinePO> = mutableListOf(),
)

@Entity @Table(name = "cart_assessment_lines", uniqueConstraints = [UniqueConstraint(columnNames = ["assessment_id", "cart_line_id"])])
class CartAssessmentLinePO(
    @Id @GeneratedValue(strategy = GenerationType.IDENTITY) var id: Long = 0,
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "assessment_id", nullable = false) var assessment: CartAssessmentPO? = null,
    @Column(name = "cart_line_id", nullable = false) var cartLineId: Long = 0,
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 32) var status: LineAssessmentStatus = LineAssessmentStatus.UNSELECTED,
...
@Entity @Table(name = "cart_request_receipts", uniqueConstraints = [UniqueConstraint(columnNames = ["buyer_id", "request_id"])])
