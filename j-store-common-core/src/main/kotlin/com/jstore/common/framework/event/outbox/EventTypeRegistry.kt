...
class InMemoryEventTypeRegistry : EventTypeRegistry {
    ...
    override fun register(eventName: String, eventVersion: Int, eventClass: Class<out DomainEvent>) {
    ...
    override fun resolve(eventName: String, eventVersion: Int): Class<out DomainEvent> {
        eventTypes[EventTypeKey(eventName, eventVersion)]?.let { return it }

        throw OutboxSerializationException("无法识别的事件类型: eventName=$eventName, eventVersion=$eventVersion")
    }
}
