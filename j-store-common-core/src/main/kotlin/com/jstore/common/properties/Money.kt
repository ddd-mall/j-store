...
interface Money<C : Currency, U : CurrencyUnit<C>> {
    ...
    fun to(otherUnit: U): Money<C, U> {
        return of(
            otherUnit,
            this.getCurrencyUnit().rateOf(otherUnit).multiply(this.getValue(), getCurrency().mathContext())
        )
    }
...
abstract class AbstractMoney<C : Currency, U : CurrencyUnit<C>> : Money<C, U> {
    ...
    override fun multiply(value: Int): Money<C, U> {
        return of(
            ...
            this.getValue()
                .multiply(BigDecimal(value, this.getCurrency().mathContext()), this.getCurrency().mathContext())
        )
    }
    ...
    override fun divide(value: Int): Money<C, U> {
        return of(
            this.getCurrencyUnit(),
            this.getValue()
                .divide(BigDecimal(value, this.getCurrency().mathContext()), this.getCurrency().mathContext())
        )
    }
    ...
    override fun sub(other: Money<C, U>): Money<C, U> {
        val otherValues = other.getCurrencyUnit()
            .rateOf(this.getCurrencyUnit())
            .multiply(other.getValue(), this.getCurrency().mathContext())
    ...
    override fun sum(other: Money<C, U>): Money<C, U> {
        val otherValues = other.getCurrencyUnit()
            .rateOf(this.getCurrencyUnit())
            .multiply(other.getValue(), getCurrency().mathContext())
