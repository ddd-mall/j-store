 ...
 *
 * For any AddressComponent and any Locale, if that Locale exists in the names map, getName(locale)
 * returns the corresponding name; if it does not exist, getName(locale) falls back to the
 * defaultLocale name.
 *
 * **Validates: Requirements 3.2, 3.3**
 */
class AddressComponentPropertyTest :
    FunSpec({
        ...
        test("getName falls back to defaultLocale name when locale is NOT in names map") {
            ...
            checkAll(100, Arb.addressComponent()) { component ->
                ...
                if (missingLocales.isNotEmpty()) {
                    ...
                    component.getName(missingLocale) shouldBe
                        component.names.getValue(component.defaultLocale)
                }
            }
        }
    })
