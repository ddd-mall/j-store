 ...
 * Property 1: 值对象构造不变量
 *
 * For any legal I18nGeoAddress, its countryCode must be a valid ISO 3166-1 alpha-2 code,
 * components list must be non-empty, and each AddressComponent's code must be non-blank,
 * names must be non-empty, and defaultLocale must exist in the names map.
 *
...
class I18nGeoAddressPropertyTest : FunSpec({
    ...
    val validCountryCodes = listOf(CountryCode.CN, CountryCode.US, CountryCode.JP, CountryCode.SG)
    ...
    test("each component code is non-blank, names is non-empty, defaultLocale exists in names") {
        checkAll(100, Arb.i18nGeoAddress()) { address ->
