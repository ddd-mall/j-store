 ...
 *
 * For any legal I18nGeoAddress, serializing to JSON then deserializing back
 * should produce an object equal to the original.
 *
 * **Validates: Requirements 9.1, 9.2, 9.3, 9.4**
 */
class I18nGeoAddressSerializationPropertyTest : FunSpec({
    ...
    test("JSON deserialization with missing required fields produces descriptive error") {
        ...
        checkAll(100, Arb.i18nGeoAddress(), Arb.element(fieldsToRemove)) { address, fieldToRemove ->
            val fullJson = objectMapper.writeValueAsString(address)
            val jsonTree = objectMapper.readTree(fullJson) as com.fasterxml.jackson.databind.node.ObjectNode
