...
class MicrometerOutboxMonitor(
    ...
    init {
        OutboxEntryStatus.entries.forEach { status ->
            meterRegistry.gauge("jstore.outbox.entries", listOf(Tag.of("status", status.name)), status) {
                outboxEntryRepository.countByStatus(it).toDouble()
            }
        }
        meterRegistry.gauge("jstore.outbox.oldest_ready.lag", operationalHealth) {
            it.snapshot().oldestReadyLag.toMillis() / 1000.0
        }
        meterRegistry.gauge("jstore.outbox.expired_locks", operationalHealth) { it.snapshot().expiredLockCount.toDouble() }
        ...
        meterRegistry.gauge("jstore.outbox.scheduler.last_success", schedulerState) {
            it.snapshot().lastSuccessAt?.epochSecond?.toDouble() ?: 0.0
        }
        meterRegistry.gauge("jstore.outbox.scheduler.last_failure", schedulerState) {
            it.snapshot().lastFailureAt?.epochSecond?.toDouble() ?: 0.0
        }
        meterRegistry.gauge("jstore.outbox.scheduler.consecutive_failures", schedulerState) {
    ...
    private fun registerAlertGauge(reason: String, active: (OutboxOperationalSnapshot) -> Boolean) {
        meterRegistry.gauge("jstore.outbox.alert", listOf(Tag.of("reason", reason)), operationalHealth) {
