...
class SpringEventTypeRegistryRegistrar(
    ...
    override fun afterSingletonsInstantiated() {
        val scanner = object : ClassPathScanningCandidateComponentProvider(false) {
            override fun isCandidateComponent(beanDefinition: org.springframework.beans.factory.annotation.AnnotatedBeanDefinition): Boolean {
        ...
        var registered = 0
        scanPackages.asSequence()
            .map(String::trim)
            .filter(String::isNotEmpty)
            .flatMap { scanner.findCandidateComponents(it).asSequence() }
            .forEach { candidate ->
                val eventClass = loadEventClass(
                    candidate.beanClassName
                        ?: throw IllegalStateException("@DomainEventType candidate has no beanClassName: $candidate")
                )
                val eventType = eventClass.getAnnotation(DomainEventType::class.java)
                    ?: throw IllegalStateException("@DomainEventType candidate is missing runtime annotation: ${eventClass.name}")
        ...
        logger.info("Domain event types registered: count={}, packages={}", registered, scanPackages)
    }

    @Suppress("UNCHECKED_CAST")
    private fun loadEventClass(className: String): Class<out DomainEvent> {
        val clazz = runCatching { Class.forName(className) }.getOrElse { cause ->
            throw IllegalStateException("Failed to load @DomainEventType class: $className", cause)
        }
