 ...
 */
class JacksonEventSerializerPropertyTest :
    FunSpec({
        val objectMapper =
            ObjectMapper()
        ...
        val arbOrderCreatedEvent: Arb<DomainEvent> =
            Arb.bind(
                arbOrderId,
                arbPrice,
                arbItemList,
                ...
                OrderCreatedEvent(id, MerchantId(7), amount, "CNY", items, ts)
            }

        val arbOrderPaidEvent: Arb<DomainEvent> =
            Arb.bind(
                arbOrderId,
                arbPrice,
                arbItemList,
        ...
        val arbOrderCompletedEvent: Arb<DomainEvent> =
            Arb.bind(
                arbOrderId,
                arbInstant,
            ) { id, ts ->
                OrderCompletedEvent(id, ts)
            }

        val arbOrderCancelledEvent: Arb<DomainEvent> =
            Arb.bind(
                arbOrderId,
                Arb.string(0..50),
                arbInstant,
            ) { id, reason, ts ->
                OrderCancelledEvent(id, reason, ts)
            }
        ...
        test("Property 2: serialize then deserialize produces equivalent object") {
            checkAll(PropTestConfig(iterations = 20), arbDomainEvent) { event ->
                ...
                val restored =
                    serializer.deserialize(json, metadata.eventName, metadata.eventVersion)
