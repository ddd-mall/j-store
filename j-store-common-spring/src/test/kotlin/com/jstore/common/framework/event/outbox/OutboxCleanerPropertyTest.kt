...
class OutboxCleanerPropertyTest : FunSpec({
    ...
    fun arbOutboxEntry(
        ...
        createdAt: Arb<Instant> = Arb.long(1_000_000L..2_000_000_000L).map { Instant.ofEpochSecond(it) }
    ): Arb<OutboxEntry> = Arb.bind(
        arbId, arbEventType, arbPayload, arbAggregateType, arbAggregateId, status, createdAt, retryCount
        ...
        checkAll(
            ...
            val eligible = entries.filter { entry ->
                entry.status == OutboxEntryStatus.PENDING ||
                    (entry.status == OutboxEntryStatus.FAILED && entry.retryCount < maxRetryCount)
            }
            ...
            result.forEach { entry ->
                val isEligible = entry.status == OutboxEntryStatus.PENDING ||
                    (entry.status == OutboxEntryStatus.FAILED && entry.retryCount < maxRetryCount)
    ...
    test("Property 8: cleanup only deletes PUBLISHED entries older than retention period") {
        checkAll(
            ...
            val shouldBeDeleted = entries.filter { entry ->
                entry.status == OutboxEntryStatus.PUBLISHED && entry.createdAt.isBefore(retentionCutoff)
            }
            ...
            val properties = OutboxProperties(retentionDays = retentionDays, cleanupBatchSize = 500)
            ...
            entries.forEach { entry ->
                val matchesDeletion = entry.status == OutboxEntryStatus.PUBLISHED &&
