 ...
 */
class OutboxEventPublisherPropertyTest : FunSpec({
    ...
    val arbDomainEvent: Arb<DomainEvent> = Arb.choice(
        Arb.bind(arbOrderId, arbPrice, arbItemList, arbInstant) { id, amount, items, ts ->
            OrderCreatedEvent(id, amount, items, ts) as DomainEvent
        },
        Arb.bind(arbOrderId, arbPrice, arbItemList, arbInstant) { id, amount, items, ts ->
            OrderPaidEvent(id, amount, items, ts) as DomainEvent
        },
        Arb.bind(arbOrderId, arbInstant) { id, ts -> OrderShippedEvent(id, ts) as DomainEvent },
        Arb.bind(arbOrderId, arbInstant) { id, ts -> OrderCompletedEvent(id, ts) as DomainEvent },
