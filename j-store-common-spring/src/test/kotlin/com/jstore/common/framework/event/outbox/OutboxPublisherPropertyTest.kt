 ...
 */
class OutboxPublisherPropertyTest : FunSpec({
    ...
    fun arbOutboxEntry(
        ...
        arbId, arbEventType, arbPayload, arbAggregateType, arbAggregateId, createdAt
    ...
    test("Property 5: batch size limits the number of entries per poll") {
        checkAll(
            ...
            val allEntries = (1..totalEntries).map { i ->
                OutboxEntry(
                    id = "id-$i",
    ...
    test("Property 6: failure handling and dead letter conversion") {
        checkAll(
            ...
            val entry = OutboxEntry(
                ...
                aggregateType = "Order",
                aggregateId = "1",
                status = if (retryCount == 0) OutboxEntryStatus.PENDING else OutboxEntryStatus.FAILED,
