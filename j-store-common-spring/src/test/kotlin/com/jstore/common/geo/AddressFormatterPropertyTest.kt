 ...
 *
 * For any valid Chinese I18nGeoAddress, formatting with ChinaAddressTemplate produces a string
 * where administrative divisions appear in depth-ascending order (省→市→区/县). To verify order: for
 * each pair of consecutive components sorted by depth, the name of the lower-depth component
 * appears before the higher-depth component in the formatted string.
 *
 * **Validates: Requirements 4.2, 4.3, 4.4**
 */
class AddressFormatterPropertyTest :
    FunSpec({
        ...
        /**
         * Generates a Chinese I18nGeoAddress with 1-3 components at distinct depth levels (1, 2,
         * 3), each with a unique name drawn from level-specific pools. Components are shuffled to
         * ensure the formatter (not the input order) determines the output order.
         */
        ...
        test("Property 4: Chinese address formatting produces depth-ascending order (省→市→区/县)") {
            ...
            checkAll(100, arbChineseAddress()) { address ->
                ...
                // For each consecutive pair, verify the lower-depth name appears before the
                // higher-depth name
        ...
        val enProvinceNames =
            listOf("Beijing", "Shanghai", "Guangdong", "Zhejiang", "Jiangsu", "Sichuan")
        ...
        /**
         * Generates a Chinese I18nGeoAddress with multi-locale names (zh-CN and en-US). Each
         * component has names in both locales, with zh-CN as the default.
         */
        ...
        test(
            "Property 5: formatted string contains each component name in the specified Locale (or fallback)"
        ) {
            // Feature: geo-address-i18n, Property 5: 格式化使用指定 Locale 的名称
            checkAll(100, arbMultiLocaleAddress(), arbTargetLocale()) { address, targetLocale ->
                ...
                // Every component's name for the target locale (or fallback) must appear in the
                // result
