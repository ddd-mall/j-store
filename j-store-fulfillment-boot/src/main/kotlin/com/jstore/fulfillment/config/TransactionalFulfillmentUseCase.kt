...
class TransactionalFulfillmentUseCase(
    ...
    override fun createForOrder(request: FulfillmentRequest) = tx { delegate.createForOrder(request) }
    override fun getByOrderId(orderId: Long) = query { delegate.getByOrderId(orderId) }
    override fun prepare(orderId: Long, occurredAt: Instant) = tx { delegate.prepare(orderId, occurredAt) }
    override fun dispatch(orderId: Long, carrierCode: String, trackingNumber: String, occurredAt: Instant) = tx { delegate.dispatch(orderId, carrierCode, trackingNumber, occurredAt) }
    override fun deliver(orderId: Long, occurredAt: Instant) = tx { delegate.deliver(orderId, occurredAt) }
