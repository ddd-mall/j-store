...
class CommodityService(
  ...
  override fun publishDraft(draftSpuId: SpuId): Result<SpuSnapshot, BusinessError> {
    val draft = spuRepository.findById(draftSpuId) ?: return Failure(CommodityErrors.SPU_NOT_FOUND)
    if (draft.sourceSpuId == null) {
      return Failure(CommodityErrors.NOT_A_DRAFT_COPY)
    }
    val source =
        spuRepository.findById(draft.sourceSpuId!!) ?: return Failure(CommodityErrors.SPU_NOT_FOUND)
