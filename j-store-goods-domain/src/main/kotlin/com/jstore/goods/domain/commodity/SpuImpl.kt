...
class SpuImpl(
  ...
  override fun addSku(sku: Sku): Result<Unit, BusinessError> {
    if (_status != CommodityStatus.DRAFT) return Failure(CommodityErrors.SKU_DIRECT_EDIT_REJECTED)
  ...
  override fun archive(): Result<Unit, BusinessError> {
    if (_status != CommodityStatus.PUBLISHED) {
      return Failure(CommodityErrors.INVALID_STATUS_TRANSITION.msg("只有已发布商品可以归档，当前状态: $_status"))
    }
  ...
  override fun mergeFromDraft(draft: Spu): Result<Unit, BusinessError> {
    ...
    if (_status != CommodityStatus.PUBLISHED) {
      return Failure(CommodityErrors.INVALID_STATUS_TRANSITION.msg("只有已发布商品可以合并草稿，当前状态: $_status"))
    }
