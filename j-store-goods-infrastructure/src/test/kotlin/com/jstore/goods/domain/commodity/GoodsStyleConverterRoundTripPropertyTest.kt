 ...
 *
 * For any 有效的 GoodsStyle 领域对象，经过 Converter.toPO 转换为 GoodsStylePO 再经过 Converter.toDomain
 * 转换回领域对象，结果应与原始对象在所有业务字段上等价 （id、spuId、mainImages、detailHtml、skuImages）。
 *
 * **Validates: Requirements 5.6, 5.7**
 */
class GoodsStyleConverterRoundTripPropertyTest :
    FunSpec({
        ...
        // Generator for skuImages: Map<SkuId, List<String>> with distinct keys and distinct image
        // values per key
        val skuImagesArb: Arb<Map<SkuId, List<String>>> =
            Arb.list(
                    Arb.bind(skuIdArb, distinctImageKeysArb(0..4)) { skuId, images ->
                        skuId to images
                    },
                    0..3,
                )
        ...
        val goodsStyleArb: Arb<GoodsStyle> =
            Arb.bind(
        ...
        test("toPO then toDomain should preserve all business fields") {
            checkAll(100, goodsStyleArb) { goodsStyle ->
                ...
                for ((skuId, images) in goodsStyle.skuImages) {
                    val roundTrippedImages =
                        roundTripped.skuImages.entries.find { it.key.value == skuId.value }?.value
