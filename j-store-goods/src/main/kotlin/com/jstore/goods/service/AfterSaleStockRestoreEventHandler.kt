...
class AfterSaleStockRestoreEventHandler(
 ...
 override fun onDomainEvent(event:AfterSaleStockRestoreRequestedEvent){
  val inventoryService = inventoryServiceProvider()
   ?: throw IllegalStateException("inventory service is not configured")
  event.items.forEach{inventoryService.add(CommodityCode(it.skuId),BigDecimal(it.quantity)).onFailure{throw IllegalStateException("after-sale stock restore failed: ${it.errorCode}")}}
 }
}
