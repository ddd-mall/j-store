 ...
 * Property 2: 重复图片标识拒绝
 *
 * For any 包含至少一个重复元素的 ImageKey 列表，调用 updateMainImages 或 updateSkuImages
 * 应返回 Failure，且 GoodsStyle 的原有状态不变。
 *
 * **Validates: Requirements 2.4, 4.4**
 */
class GoodsStyleDuplicateImageRejectionPropertyTest : FunSpec({

    // Generator: produces a list of ImageKeys that contains at least one duplicate
    val duplicateImageKeysArb: Arb<List<String>> = Arb.bind(
    ...
    fun createGoodsStyle(
        ...
        skuImages: Map<SkuId, List<String>> = mapOf(SkuId(99L) to listOf("sku-img-1", "sku-img-2")),
    ): GoodsStyleImpl {
    ...
    test("updateMainImages should return Failure and leave mainImages unchanged when given duplicate ImageKeys") {
    ...
    test("updateSkuImages should return Failure and leave skuImages unchanged when given duplicate ImageKeys") {
