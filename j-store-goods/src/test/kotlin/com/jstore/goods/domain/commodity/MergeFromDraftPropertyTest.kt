 ...
 *
 * For any ON_SALE 状态的源 SPU 和任意包含至少一个 SKU 的草稿 SPU，
 * 调用 source.mergeFromDraft(draft) 后，源 SPU 应满足：
 * name 等于草稿的 name，description 等于草稿的 description，
 * SKU 列表与草稿的 SKU 列表内容一致，version 等于合并前的 version + 1，
 * status 保持 ON_SALE 不变。
 *
 * **Validates: Requirements 6.2, 6.3, 6.4, 9.2, 9.3, 9.4**
 */
class MergeFromDraftPropertyTest : FunSpec({

    // Generator for a single SKU with unique attribute value
    fun skuArb(attrValue: String): Arb<Sku> = Arb.bind(
    ...
    val draftSpuArb: Arb<SpuImpl> = Arb.bind(
        Arb.long(1L..Long.MAX_VALUE),
        Arb.string(1..50),
        Arb.string(0..100),
        skuListArb,
