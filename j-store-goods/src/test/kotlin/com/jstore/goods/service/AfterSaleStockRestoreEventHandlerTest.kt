...
class AfterSaleStockRestoreEventHandlerTest :
    FunSpec({
        test("restore contract preserves sku and exact quantity") {
            val event = AfterSaleStockRestoreRequestedEvent(1, 2, listOf(StockRestoreItem(3, 4)))
            event.eventName shouldBe "stock.after-sale-restore-requested"
            event.items.single() shouldBe StockRestoreItem(3, 4)
        }
        test("handler adds the exact approved quantity once") {
            ...
            AfterSaleStockRestoreEventHandler(inventory)
                .onDomainEvent(
                    AfterSaleStockRestoreRequestedEvent(1, 2, listOf(StockRestoreItem(3, 4)))
                )
