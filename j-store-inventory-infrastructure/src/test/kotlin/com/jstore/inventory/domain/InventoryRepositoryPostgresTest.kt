...
class InventoryRepositoryPostgresTest {
  ...
  fun `stock position and reservation survive a PostgreSQL round trip`() = database { factory ->
    transaction(factory) { entityManager ->
      ...
      val reservation =
          assertNotNull(repositories.reservations.findByBusinessKey("ORDER-51-SKU-41-NODE-DEFAULT"))
      ...
      database { factory ->
        ...
        try {
          val first =
              pool.submit<Boolean> {
                transaction(factory) { entityManager ->
                  ...
                  val locked = positions.lock(listOf(StockPositionId("41@DEFAULT"))).single()
          ...
          transaction(factory) {
            assertEquals(
                9,
                repositories(it).positions.findById(StockPositionId("41@DEFAULT"))?.reserved,
            )
          }
        } finally {
          allowFirstCommit.countDown()
