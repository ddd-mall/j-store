...
class StockRepositoryImpl(
    ...
    override fun findByOrderIdAndGoodsId(orderId: SaleOrderId, goodsId: GoodsId): Stock? {
        return stockPOJpaRepository.findStockPOByOrderIdAndSpuIdAndSkuId(orderId.value, goodsId.skuId, goodsId.spuId)
            ?.toStock()
    }

    override fun saveBatch(stocks: Collection<Stock>) : List<Stock> {
        val poList = stocks.map { StockPO(it) }.toList()
        return stockPOJpaRepository.saveAll(poList).map { it.toStock() }
    }

    override fun save(entity: Stock): Stock {
        val po = StockPO(entity)
        return stockPOJpaRepository.save(po).toStock()
    }

    override fun findById(id: StockId): Stock? {
        return stockPOJpaRepository.findById(id.value).map { it.toStock() }.orElse(null)
    }
}