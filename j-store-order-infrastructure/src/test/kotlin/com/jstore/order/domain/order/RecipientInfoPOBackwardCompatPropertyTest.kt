 ...
 *
 * For any 合法的 RecipientInfoPO JSON，当 consigneeName 字段缺失或为 null 时，
 * 反序列化后重建的 ShippingInfo.consigneeName 应为空字符串 ""；
 * 当 consigneePhone 和 consigneeEmail 均缺失或为 null 时，
 * 重建的 ContractInfo 的 phoneNumber 和 email 均应为 null。
 *
 * **Validates: Requirements 6.1, 6.2**
 */
class RecipientInfoPOBackwardCompatPropertyTest : FunSpec({
    ...
    test("consigneeName null defaults to empty string in ShippingInfo") {
        checkAll(100, i18nGeoAddressArb, digitCodeArb, optionalDetailAddressArb) { address, districtCode, detailAddress ->
            // Simulate historical data: consigneeName is null
            val historicalPO = RecipientInfoPO(
                consigneeName = null,
    ...
    test("consigneePhone and consigneeEmail both null defaults to ContractInfo with null phoneNumber and null email") {
        checkAll(100, i18nGeoAddressArb, digitCodeArb, nonBlankStringArb, optionalDetailAddressArb) { address, districtCode, consigneeName, detailAddress ->
