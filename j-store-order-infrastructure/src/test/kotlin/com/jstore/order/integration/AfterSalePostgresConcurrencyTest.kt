...
class AfterSalePostgresConcurrencyTest {
    ...
    fun `capacity lock serializes concurrent reservations without overselling`() = database { pg ->
        ...
        try {
            val results =
                (1..2)
                    .map { pool.submit(Callable { reserve(pg, listOf(11), 1) }) }
            ...
            pg.connection.use { c ->
                assertEquals(
                    ...
                    queryInt(
                        c,
                        "select requested_quantity from after_sale_capacities where order_item_id=11",
                    ),
                )
            }
        } finally {
    ...
    @Test
    fun `sorted multi-item locking completes in opposite request order without deadlock`() =
        database { pg ->
            seedCapacity(pg, 21, 2)
            seedCapacity(pg, 22, 2)
            val pool = Executors.newFixedThreadPool(2)
            try {
    ...
    fun `optimistic decision and idempotency keys have one winner`() = database { pg ->
        pg.connection.use { c ->
            c.createStatement().use {
                it.executeUpdate(
                    "insert into after_sales(id,order_id,applicant_id,merchant_id,status,reason_category,reason_description,fulfillment_status,require_return,create_time,update_time,version) values(1,1,1,7,'REQUESTED','OTHER','x','UNFULFILLED',false,now(),now(),0)"
                )
            }
        }
        val pool = Executors.newFixedThreadPool(2)
        try {
            val decisions =
                (1..2)
                    .map { pool.submit(Callable { updateDecision(pg) }) }
                    .map { it.get(10, TimeUnit.SECONDS) }
            assertEquals(1, decisions.count { it })
            val receipts =
                (1..2)
                    .map { i -> pool.submit(Callable { insertReceipt(pg, i.toLong()) }) }
                    .map { it.get(10, TimeUnit.SECONDS) }
    ...
    fun `receipt and outbox are rolled back together`() = database { pg ->
        ...
        pg.connection.use { c ->
            assertEquals(
                0,
                queryInt(c, "select count(*) from after_sale_command_receipts where id=99"),
            )
            assertEquals(
                0,
                queryInt(c, "select count(*) from outbox_entry where event_id='event-99'"),
            )
        }
    }
    ...
    private fun seedCapacity(ds: javax.sql.DataSource, id: Long, ceiling: Int) =
        ds.connection.use { c ->
            c.createStatement().use {
                it.executeUpdate(
                    "insert into after_sale_capacities(order_item_id,order_id,quantity_ceiling,amount_ceiling,requested_quantity,requested_amount,approved_quantity,approved_amount,version) values($id,1,$ceiling,100,0,0,0,0,0)"
                )
            }
        }
    ...
    private fun reserve(
        ...
        ds.connection.use { c ->
            ...
            try {
                ...
                c.prepareStatement(
                        "select order_item_id from after_sale_capacities where order_item_id in (${ids.joinToString()}) order by order_item_id for update"
                    )
                    .use { it.executeQuery().use { rs -> while (rs.next()) Unit } }
                val available = ids.all {
                    queryInt(
                        c,
                        "select quantity_ceiling-requested_quantity-approved_quantity from after_sale_capacities where order_item_id=$it",
                    ) >= quantity
                }
                if (available)
                    ids.forEach {
                        c.createStatement().use { s ->
                            s.executeUpdate(
                                "update after_sale_capacities set requested_quantity=requested_quantity+$quantity where order_item_id=$it"
                            )
                        }
                    }
                c.commit()
    ...
    private fun updateDecision(ds: javax.sql.DataSource): Boolean =
        ds.connection.use { c ->
            c.createStatement().use {
                it.executeUpdate(
                    "update after_sales set status='APPROVED',version=version+1 where id=1 and version=0"
                ) == 1
            }
        }

    private fun insertReceipt(ds: javax.sql.DataSource, id: Long): Boolean =
        try {
            ds.connection.use { c ->
                c.createStatement().use {
                    it.executeUpdate(
                        "insert into after_sale_command_receipts values($id,1,'CREATE','same','hash',1,'REQUESTED',now())"
                    )
                }
            }
            true
    ...
    private fun queryInt(c: Connection, sql: String): Int =
        c.createStatement().use { s ->
