 ...
 *
 * For any OrderCreateCMD，其中至少一个 OrderItemCMD 的 snapshotVersion 与 GoodsService 返回的最新
 * snapshotVersion 不一致，OrderFactory.create 应返回 Failure（SNAPSHOT_VERSION_MISMATCH），
 * 且错误信息中包含不匹配的商品 SPU ID。
 *
 * **Validates: Requirements 10.2, 10.3, 11.2**
 */
class SnapshotVersionMismatchPropertyTest : FunSpec({
    ...
    val stubGeoAddressService = object : GeoAddressService {
    ...
    val nonZeroOffsetArb: Arb<Long> = Arb.choice(
        Arb.long(1L..100L),
        Arb.long(-100L..-1L),
    )

    test("OrderFactory.create() returns Failure(SNAPSHOT_VERSION_MISMATCH) when snapshotVersion mismatches") {
        checkAll(100, spuIdArb, goodsVersionArb, nonZeroOffsetArb) { spuId, goodsVersion, offset ->
            ...
            val factory = OrderFactoryImpl(snowFlakSequence, goodsService, stubGeoAddressService)
