 ...
 *
 * For any 会导致 RecipientInfoCMD.validate() 失败的输入组合，
 * OrderCreateCMD.validate() 也应返回 Failure，且错误信息与 RecipientInfoCMD.validate() 返回的一致。
 *
 * **Validates: Requirements 3.4**
 */
class OrderCreateCMDValidationPropertyTest : FunSpec({

    val validPhone = PhoneNumber("13800138000")
    /**
     * Generator for RecipientInfoCMD instances that will fail validation.
     ...
     */
    val failingRecipientInfoArb: Arb<OrderCreateCMD.RecipientInfoCMD> = Arb.element(1, 2, 3).map { mode ->
        when (mode) {
            1 -> OrderCreateCMD.RecipientInfoCMD(
                consigneeName = "  ",
                countryCode = "CN",
                consigneeContractInfo = OrderCreateCMD.ContractInfoCMD(phoneNumber = validPhone),
                shippingDistrictCode = "110105",
                shippingDetailAddress = "详细地址",
            )
            2 -> OrderCreateCMD.RecipientInfoCMD(
                consigneeName = "张三",
                countryCode = "CN",
                consigneeContractInfo = OrderCreateCMD.ContractInfoCMD(phoneNumber = validPhone),
            ...
            else -> OrderCreateCMD.RecipientInfoCMD(
                consigneeName = "张三",
                countryCode = "CN",
                consigneeContractInfo = OrderCreateCMD.ContractInfoCMD(phoneNumber = null, emailAddress = null),
                shippingDistrictCode = "110105",
                shippingDetailAddress = "详细地址",
            )
        }
    ...
    test("OrderCreateCMD.validate() propagates the same Failure as RecipientInfoCMD.validate()") {
