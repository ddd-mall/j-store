...
class TradePaymentCancellationService(
  ...
  fun start(
    ...
    if (payment.status == TradePaymentStatus.PREPARATION_CANCELLING) {
      return Success(TradePaymentCancellationStart.Completed(changed is Success && changed.value))
    }
    return Success(
...
class TradePaymentPreparationService(
  ...
  fun start(
    ...
    if (payment.status != TradePaymentStatus.PREPARING) {
      if (payment.status !in setOf(TradePaymentStatus.CANCELLED, TradePaymentStatus.CANCELLING)) {
      ...
      is PaymentProviderResult.Accepted ->
          if (
              result.acceptedAt <= command.acceptBefore &&
                  ...
                  result.providerReference.length <= TradePayment.MAX_PROVIDER_REFERENCE_LENGTH &&
