...
class TradePaymentPreparationServiceTest {
  ...
  fun `confirmed retry publishes the original cancellation reason`() {
    ...
    val event = assertIs<PaymentCancellationConfirmedIntegrationEvent>(publisher.messages.single())
  ...
  fun `invalid accepted result racing cancellation remains cancellable`() {
    ...
    val oversizedReference = "provider-" + "r".repeat(TradePayment.MAX_PROVIDER_REFERENCE_LENGTH)
