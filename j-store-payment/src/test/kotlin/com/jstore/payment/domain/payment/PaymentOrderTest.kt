...
class PaymentOrderTest : FunSpec({
    ...
    test("capture requires the frozen full amount and is idempotent by provider transaction") {
        val payment = payment()
        (payment.capture("txn-1", Price.ofFen(900), "CNY", Instant.EPOCH) is Failure) shouldBe true
        payment.status shouldBe PaymentOrderStatus.PENDING

        (payment.capture("txn-1", Price.ofFen(1_000), "CNY", Instant.EPOCH) as Success).value shouldBe true
        ...
        (payment.capture("txn-1", Price.ofFen(1_000), "CNY", Instant.EPOCH) as Success).value shouldBe false
    }
