...
class OfferRepositoryPostgresTest {
  ...
  fun `store offer and authorization survive a PostgreSQL round trip`() = database { factory ->
    transaction(factory) { entityManager ->
      ...
      repositories.stores.save(Store(StoreId(11), MerchantId(21), "Main Store", StoreStatus.ACTIVE))
  ...
  @Test
  fun `offer lock exposes a committed suspension to the next transaction`() = database { factory ->
    transaction(factory) { entityManager ->
      val repositories = repositories(entityManager)
      repositories.stores.save(Store(StoreId(11), MerchantId(21), "Main Store", StoreStatus.ACTIVE))
      ...
      val second =
          pool.submit<OfferStatus> {
            transaction(factory) { entityManager ->
              ...
              repositories(entityManager).offers.lock(listOf(SalesOfferId(12))).single().status
            }
          }
