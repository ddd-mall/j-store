...
class CheckoutApplicationService(
    ...
    private val source: CheckoutSourceGateway = CheckoutSourceGateway { command ->
      if (command.cartId == null) Success(command) else Failure(TradeErrors.CHECKOUT_OFFER_INVALID)
    },
) : CheckoutUseCase {
  override fun checkout(command: CreateCheckoutCommand): Result<CheckoutAccepted, BusinessError> {
    ...
    if (command.cartId != null) {
      trades.findByCheckoutRequest(requestedPrincipal, command.checkoutRequestId)?.let { existing ->
  ...
  override fun find(
    ...
    if (trade.actingPrincipal != AuthenticatedAccountSnapshot(buyerAuthenticationDomain, buyerId)) {
