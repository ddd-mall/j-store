...
interface TradeSagaUseCase {
  ...
  fun recordPaymentPrepared(event: PaymentPreparedIntegrationEvent): Result<Boolean, BusinessError>
        ...
        if (trade.status == TradeStatus.FAILED) {
          ...
          if (
              authorizationIds.any { it.isBlank() } ||
                  event.items.map { it.offerId }.toSet() != plan.items.map { it.offerId }.toSet()
          ) {
