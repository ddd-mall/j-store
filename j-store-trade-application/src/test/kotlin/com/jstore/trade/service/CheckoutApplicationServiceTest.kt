...
class CheckoutApplicationServiceTest {
  ...
  fun `checkout creates an independent trade and plans before orders exist`() {
    ...
    val accepted = assertIs<Success<CheckoutAccepted>>(service.checkout(checkoutCommand())).value
  ...
  fun `delimiter characters cannot make different checkout requests share a digest`() {
    ...
    val first =
        checkoutCommand()
            .copy(
                recipient = checkoutCommand().recipient.copy(name = "Alice|CN", countryCode = "US")
            )
  ...
  fun `oversized public checkout fields are rejected before preparation`() {
    ...
    val invalidCommands =
        listOf(
            ...
            command.copy(
                recipient = command.recipient.copy(customsFields = mapOf("x".repeat(129) to "v"))
            ),
        )
