...
class MultiMerchantCartCheckoutTest {
    ...
    fun `two merchants three items create one trade and two amount-conserving plans`() {
        ...
        val command = CreateCheckoutCommand("cart-checkout", 42, CheckoutRecipient("张三", "CN", "13800000000", null, "110105", "示例路"), cartId = 9, expectedCartVersion = 4)
        val items = listOf(CheckoutItem(11, 1, 101, 1001, 1, 1), CheckoutItem(12, 1, 102, 1002, 2, 1), CheckoutItem(21, 1, 201, 2001, 1, 1))
        val ids = ArrayDeque(listOf(9001L, 9002L, 8001L))
        val service = CheckoutApplicationService(
        ...
        trade.orderPlans.forEachIndexed { index, plan ->
            trade.recordSaleAuthorized(plan.id, plan.items.map { TradeAuthorization("A-$index-${it.offerId}", it.offerId, expiresAt) })
            trade.recordInventoryReserved(plan.id, listOf("R-$index"), expiresAt)
        }
        trade.startOrderCreation()
        trade.orderPlans.forEachIndexed { index, plan -> trade.recordOrderCreated(plan.id, 7001L + index) }
        assertEquals(listOf(7001L, 7002L), trade.orderPlans.mapNotNull { it.orderId })
    }

    private fun prepared(command: CreateCheckoutCommand): PreparedCheckout = PreparedCheckout(
        command,
        command.items.mapIndexed { index, item ->
            val merchant = if (index < 2) 7L else 8L
            val node = if (index < 2) "NODE-A" else "NODE-B"
            PreparedCheckoutItem(item.offerId, merchant, merchant, item.spuId, item.skuId, item.quantity, item.catalogSnapshotVersion, item.offerVersion, node, "ONLINE", Price.ofFen(if (index == 0) 500 else 1000), "商品", "规格")
        },
        TradeBuyerProfileSnapshot("张三", "13800000000"),
        I18nGeoAddress(CountryCode.CN, listOf(AddressComponent("110105", DivisionLevel(3, "district"), mapOf(Locale.CHINA to "朝阳区"), Locale.CHINA))),
        "CNY",
    )
}
...
private class MultiMerchantTradeRepository : TradeRepository {
    ...
    override fun findByCheckoutRequest(buyerParty: BuyerPartySnapshot, checkoutRequestId: String) = values.values.firstOrNull { it.buyerParty == buyerParty && it.checkoutRequestId == checkoutRequestId }
    override fun findByOrderPlanId(orderPlanId: TradeOrderPlanId) = values.values.firstOrNull { trade -> trade.orderPlans.any { it.id == orderPlanId } }
}
