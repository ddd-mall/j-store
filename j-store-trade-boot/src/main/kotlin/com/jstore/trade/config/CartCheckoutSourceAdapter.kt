...
class CartCheckoutSourceAdapter(private val carts: CartCheckoutSourceQueryService) :
    CheckoutSourceGateway {
    override fun resolve(
        command: CreateCheckoutCommand
    ): Result<CreateCheckoutCommand, com.jstore.common.errors.BusinessError> {
        val cartId =
            command.cartId
                ?: return if (command.items.isNotEmpty()) Success(command)
                else Failure(TradeErrors.CHECKOUT_REQUEST_INVALID)
        val expectedVersion =
            command.expectedCartVersion ?: return Failure(TradeErrors.CHECKOUT_REQUEST_INVALID)
        if (command.items.isNotEmpty()) return Failure(TradeErrors.CHECKOUT_REQUEST_INVALID)
        return when (
            val result =
                carts.prepare(CartCheckoutSourceQuery(command.buyerId, cartId, expectedVersion))
        ) {
            is CartCheckoutSourceResult.Found ->
                Success(
                    command.copy(
                        items =
                            result.source.eligibleLines.map {
                                CheckoutItem(
                                    it.offerId,
                                    it.offerVersion,
                                    it.spuId,
                        ...
                        cartDigest = result.source.cartDigest,
                    )
                )
