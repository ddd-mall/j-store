...
class TransactionalUserAccountUseCase(
    ...
    override fun login(phoneNumber: PhoneNumber, rawPassword: String) =
        tx { delegate.login(phoneNumber, rawPassword) }.also { result ->
            if (result is Success) {
                tokenProvider.parseRefreshToken(result.value.refreshToken)?.let { userId ->
                    tokenStore.storeRefreshToken(userId, result.value.refreshToken, REFRESH_TOKEN_TTL_SECONDS)
                }
            }
        }
    ...
    override fun changeNickname(userId: UserId, newNickname: Nickname) =
        tx { delegate.changeNickname(userId, newNickname) }
    ...
    override fun disable(userId: UserId) =
        tx { delegate.disable(userId) }.also { result ->
    ...
    override fun forceOffline(userId: UserId, accessToken: String?) =
        tx { delegate.forceOffline(userId, accessToken) }.also { result ->
            if (result is Success) {
