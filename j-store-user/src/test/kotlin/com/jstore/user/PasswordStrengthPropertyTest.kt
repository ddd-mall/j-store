...
class PasswordStrengthPropertyTest : FunSpec({
    ...
    val tooShortArb: Arb<String> = Arb.int(2..7).flatMap { len ->
        ...
        Arb.bind(
            ...
            Arb.char('0'..'9'),
        ) { letters, digit -> (letters + digit).shuffled().joinToString("") }
    }

    // Generator: too long (> 32 chars), with both letters and digits
    val tooLongArb: Arb<String> = Arb.int(33..60).flatMap { len ->
        val letterCount = len - 1
        Arb.bind(
            Arb.list(Arb.char('a'..'z'), letterCount..letterCount),
            Arb.char('0'..'9'),
