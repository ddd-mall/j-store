 ...
 *
 * For any UserAccount，当状态为 ACTIVE 时执行 disable() 应成功且状态变为 DISABLED；
 * 当状态为 DISABLED 时执行 enable() 应成功且状态变为 ACTIVE。
 * 当状态为 ACTIVE 时执行 enable() 应返回失败；
 * 当状态为 DISABLED 时执行 disable() 应返回失败。
 *
 * **Validates: Requirements 3.1, 3.2, 3.3, 3.4**
 */
class UserAccountStatusTransitionPropertyTest : FunSpec({
